...
contactPhone.addEventListener("input", (e) => {
    ...
    if (cleanedValue === "") {
        ...
        if (hasCountryCode) {
            if (phoneLength === 12 || phoneLength === 13) { // 12 or 13 digits after +
                phoneError.innerHTML = "";
            } else {
                phoneError.innerHTML = "Phone number with country code must have 12 or 13 digits after the +";
            }
        } else {
            if (phoneLength === 10) { // 10 digits without country code
                phoneError.innerHTML = "";
            } else {
                phoneError.innerHTML = "Phone number must have 10 digits (without country code)";
            }
        }
    }
});
...
form.addEventListener("submit", function (event) {
  ...
  // Perform final validation here.  Check all fields.
